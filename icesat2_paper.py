...
class Atl03Config:
    ...
    version: str = "007"
    # Keep both left+right beams; do not select per granule using sc_orient.
...
def _infer_ground_value_from_classed_pc_flag(ds) -> int:
    """Infer numeric 'ground' value from ATL08 `classed_pc_flag` dataset attributes."""
    values = None
    ...
    if meanings:
        ...
        for idx, m in enumerate(meanings_lower):
            if m == "ground" or "ground" in m:
                ...
                # If values are missing, default to the documented ATL08 convention.
                return 1

    logging.getLogger("atl03").warning("Could not infer ATL08 ground value; assuming 1.")
    return 1
...
def _build_ground_mask_from_atl08(
    ...
    """
    Build a boolean mask (length N photons in ATL03 heights arrays) selecting 'ground' photons,
    using ATL08 `signal_photons/classed_pc_flag` coupled via (ph_segment_id, classed_pc_indx)
    back to ATL03 (segment_id, ph_index_beg).
    """
    ...
    if path_flag not in f_atl08 or path_indx not in f_atl08 or path_seg not in f_atl08:
        logger.warning(f"{beam}: ATL08 missing one of classed_pc_flag/classed_pc_indx/ph_segment_id; skipping beam.")
        return None

    ds_flag = f_atl08[path_flag]
    ground_val = _infer_ground_value_from_classed_pc_flag(ds_flag)
    ...
    # ATL03 geolocation for mapping segment_id -> photon offset.
    ...
    # Vectorized segment lookup using searchsorted (segment_id is monotonically increasing).
    ...
    # Both ph_index_beg and classed_pc_indx are documented as 1-based.
...
def read_atl03_points_from_hdf5(
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        for beam in selected_beams:
            ...
            # Ground-only selection: no extra photon-level filtering besides bbox and finite heights.
...
def main() -> int:
    ...
    # Attempt to couple each ATL03 granule with its corresponding ATL08 granule by filename.
    atl08_dir = (out_dir.parent / "atl08_output" / "raw_hdf5").resolve()
    # If the user already has the default outputs, prefer workspace-level atl08_output/raw_hdf5.
    workspace_atl08 = Path("atl08_output/raw_hdf5").resolve()
    if workspace_atl08.exists():
        atl08_dir = workspace_atl08
    atl08_map = {p.name: p for p in atl08_dir.glob("ATL08_*.h5")}
    if not atl08_map:
        logger.warning(f"No ATL08 granules found in {atl08_dir}. Ground classification will be unavailable.")
    ...
    for i, fpath in enumerate(h5_files, start=1):
        try:
            ...
            if atl08_path is None:
                logger.warning(f"{fpath.name}: could not find matching ATL08 granule ({atl08_name}) in {atl08_dir}.")
